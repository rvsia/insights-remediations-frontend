...
export const RemediationSummary = ({
    ...
    const generateNumRebootString = (num) => {
        return `${num} issue${num === 1 ? '' : 's'} require${num === 1 ? 's' : ''} reboot to remediate`;
    };
    ...
    const renderAutoReboot = (remediation) => {
        return (
            <DescriptionList className='ins-c-playbookSummary__settings' title='Autoreboot'>
                <Flex>
                    <FlexItem
        ...
        <Split>
            <SplitItem>
                <ChartDonut
                    ...
                    data={ [
                        { x: 'Resolved', y: remediation.resolved_count / remediation.issues.length },
                        { x: 'Unresolved', y: (remediation.issues.length - remediation.resolved_count) / remediation.issues.length }
                    ] }
